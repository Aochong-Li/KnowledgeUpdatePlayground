...
def process(example: Dict, tokenizer: AutoTokenizer)->Dict:
    ...
    """
    if example['sft'] is None:
        example['sft'] = ''
    ids = tokenizer.apply_chat_template(example['sft'], date_string = 'December 2026')
...
def tokenize_and_save(dataset_path:str, filename: str, tokenizer: AutoTokenizer):
    """
...
if __name__ == '__main__':
    # loading tokenizer
    for model in constants.MODEL_LIST.keys():
        if 'instruct' not in model:
            continue
        model_name = constants.MODEL_LIST[model]
        ...
        dataset_path = '/share/goyal/lio/knowledge_delta/dataset/sft/alpha_train_test'
        save_path = f'/share/goyal/lio/knowledge_delta/training/sft/{model_nickname}'

        print(f"Tokenizing for {model_name} with {model_name} tokenizer")
        tokenize_and_save(dataset_path, save_path, tokenizer)