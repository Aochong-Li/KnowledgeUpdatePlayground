...
def process(example: Dict, tokenizer: AutoTokenizer, text_key: str)->Dict:
    ...
    """
    ids = tokenizer.encode(example[text_key], add_special_tokens=True) # add_special_tokens=True to add BOS token
...
def tokenize_and_save(tokenizer: AutoTokenizer, config: TokenizeConfig, output_dir: str):
    """
    ...
    print(f"Tokenize RedPajama with {config.tokenizer_model_name}")
    # loading dataset
    ...
    filename = f'{output_dir}/bins/{config.load_dataset_path.replace("/", "_").replace("-","_")}'
    if not os.path.isdir(f"{output_dir}/bins"):
        os.mkdir(f"{output_dir}/bins")
...
if __name__ == '__main__':
    ...
    config.tokenizer_model_name = model_name
    # logging
    ...
    output_dir = '/share/goyal/lio/knowledge_delta/training/redpajama-data/mistral'

    tokenize_and_save(tokenizer, config, output_dir)