...
def tokenize_list_with_multiprior (
        knowledge_list: List[List[str]],
        prior_list: List[List[str]],
        model: str) -> tuple[List[int], List[int]]:
    
    tuple_list = list(zip(knowledge_list, prior_list))
    ...
    for data in tqdm(tuple_list):
        knowledge_chunks = data[0] # List[str]
        prior_chunks = data[1] # List[str]

        ids = [tokenizer.bos_token_id]
        type_labels = [PRIOR_TOKEN_TYPE]

        # Alternate prior and knowledge chunks
        for prior_chunk, knowledge_chunk in zip_longest(prior_chunks, knowledge_chunks):
            
            # Process prior chunk (if exists)
            if prior_chunk is not None:
                tokenized = tokenizer.encode(prior_chunk, add_special_tokens = False)
                ids.extend(tokenized)
                type_labels.extend([PRIOR_TOKEN_TYPE] * len(tokenized))
            
            # Process knowledge chunk (if exists)
            if knowledge_chunk is not None:
                tokenized = tokenizer.encode(knowledge_chunk, add_special_tokens = False)
                ids.extend(tokenized)
                type_labels.extend([KNOWLEDGE_ARTICLE_TOKEN_TYPE] * len(tokenized))
        
        # Add EOS token
        ids.append(tokenizer.eos_token_id)
        type_labels.append(PRIOR_TOKEN_TYPE)
...
def tokenize_knowledge(model_name: str):
    ...
    df = pd.read_pickle(os.path.join(ROOT, f'dataset/priorlearning/multiprior_beta_dataset_{model_name}.pickle'))
    knowledge_article, prior = df['article'], df['prior']
    nontarget_content = pd.read_pickle(os.path.join(ROOT, 'dataset/nontarget_article/nontarget_article_table.pickle'))['content']

    dir = os.path.join(ROOT, f'training/multipriorlearning_data/{model_nickname}')
    if not os.path.isdir(dir):
        os.mkdir(dir)

    all_ids, token_type = tokenize_list_with_multiprior(knowledge_list=knowledge_article, prior_list = prior, model=model)
    ...
    all_ids, token_type = tokenize_list(text_list=nontarget_content, model=model)
